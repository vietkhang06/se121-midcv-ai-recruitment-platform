      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {isLoading ? (
          <EmptyState
            ...
            description={locale === 'vi' ? 'Hệ thống đang kết nối dữ liệu đơn nộp và phân tích đối sánh...' : 'Connecting application records and analyzing match reports...'}
          />
        ) : fetchError ? (
          <EmptyState
            type="ERROR"
            title={locale === 'vi' ? 'Không thể tải lịch sử ứng tuyển' : 'Failed to load application history'}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-4 text-xs text-[#64748B] dark:text-[#94A3B8] flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {locale === 'vi' ? 'Ngày nộp:' : 'Applied on:'} {app.appliedDate}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                      ...
                      >
                        ...
                        <span>{locale === 'vi' ? 'Báo cáo đối sánh' : 'Match report'}</span>
                    ...
                    <div className="pt-3 border-t border-slate-200 dark:border-[#1E293B] space-y-4 animate-fade-in">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-mono font-bold uppercase tracking-wider text-[#0F2A52] dark:text-[#E2E8F0]">
                          {locale === 'vi' ? 'BÁO CÁO PHÂN TÍCH ĐỐI SÁNH midCV® AI' : 'midCV® AI MATCH REPORT ANALYSIS'}
                        </span>
                        <span className="text-[11px] text-[#00873D] dark:text-[#10B981] font-semibold bg-[#E8F8EE] dark:bg-[#00B14F]/15 px-2.5 py-0.5 rounded-full border border-[#00B14F]/30">
                          {locale === 'vi' ? 'Xác thực bảo vệ ứng viên' : 'Candidate Privacy Protected'}
                        </span>
                      </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#13233F] border border-slate-200 dark:border-[#1E293B] space-y-1">
                              <span className="text-[10px] font-mono uppercase text-[#64748B] dark:text-[#94A3B8]">
                                {locale === 'vi' ? 'ĐIỂM ĐỐI SÁNH TỔNG THỂ' : 'OVERALL MATCH SCORE'}
                              </span>
                              ...
                              <span className={`text-[10px] font-medium block ${isHigh ? 'text-[#00B14F] dark:text-[#10B981]' : 'text-[#64748B]'}`}>
                                {insp 
                                  ? (isHigh ? (locale === 'vi' ? 'Đạt ngưỡng tiến cử trực tiếp' : 'Direct recommendation threshold met') : (locale === 'vi' ? 'Đang trong diện xem xét' : 'Under consideration')) 
                                  : (locale === 'vi' ? 'Đang tính toán điểm...' : 'Calculating score...')}
                              </span>
                            </div>

                            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#13233F] border border-slate-200 dark:border-[#1E293B] space-y-1">
                              <span className="text-[10px] font-mono uppercase text-[#64748B] dark:text-[#94A3B8]">
                                {locale === 'vi' ? 'TRỌNG SỐ CORE JD-CV' : 'CORE JD-CV WEIGHT'}
                              </span>
                              <div className="text-2xl font-editorial font-bold text-[#0F2A52] dark:text-white">
                                {core}
                              </div>
                              <span className="text-[10px] text-[#64748B] dark:text-[#94A3B8] block">
                                {insp 
                                  ? (locale === 'vi' ? `Khớp ${insp.requiredSkillsStatus?.filter(s => s.status === 'MATCH').length || 0}/${insp.requiredSkillsStatus?.length || 0} kỹ năng bắt buộc` : `Matched ${insp.requiredSkillsStatus?.filter(s => s.status === 'MATCH').length || 0}/${insp.requiredSkillsStatus?.length || 0} required skills`) 
                                  : (locale === 'vi' ? 'Phân tích tiêu chí' : 'Analyzing criteria')}
                              </span>
                            </div>

                            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#13233F] border border-slate-200 dark:border-[#1E293B] space-y-1">
                              <span className="text-[10px] font-mono uppercase text-[#64748B] dark:text-[#94A3B8]">
                                {locale === 'vi' ? 'TÍN HIỆU GITHUB (PHỤ TRỢ)' : 'GITHUB SIGNAL (AUXILIARY)'}
                              </span>
                              <div className="text-2xl font-editorial font-bold text-[#0F2A52] dark:text-white">
                                {gh}
                              </div>
                              <span className="text-[10px] text-[#64748B] dark:text-[#94A3B8] block">
                                {insp?.githubScoreActive 
                                  ? (locale === 'vi' ? 'Tín hiệu bổ trợ (15% trọng số)' : 'Auxiliary signal (15% weight)') 
                                  : (locale === 'vi' ? 'Không áp dụng (Zero Penalty)' : 'Not applied (Zero Penalty)')}
                              </span>
                            </div>
                          </div>
                        );
                      ...
                      <div className="text-xs text-[#64748B] dark:text-[#94A3B8] bg-slate-50 dark:bg-[#13233F] p-3.5 rounded-xl border border-slate-200 dark:border-[#1E293B] leading-relaxed">
                        <strong>{locale === 'vi' ? 'Lưu ý bảo mật & minh bạch:' : 'Security & Transparency Notice:'}</strong> {locale === 'vi' ? `Snapshot CV phiên bản v${app.appliedCvVersion}.0 đã được cố định tại thời điểm nộp đơn. Bất kỳ chỉnh sửa nào trong tương lai đối với hồ sơ gốc của bạn sẽ không làm thay đổi bản lưu hồ sơ này mà Nhà tuyển dụng đang xem xét.` : `CV Snapshot version v${app.appliedCvVersion}.0 was frozen upon submission. Any subsequent edits to your original profile will not alter this review record.`}
                      </div>
                    </div>
                  )}
                </div>
          ...
          <EmptyState
            ...
            primaryCtaHref="/jobs"
            secondaryCtaText={locale === 'vi' ? 'Tải lên CV kiểm tra' : 'Upload CV for check'}
