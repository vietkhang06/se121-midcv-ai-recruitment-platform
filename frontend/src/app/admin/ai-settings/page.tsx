      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-6">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
          ...
          >
            ...
            <span>{t('admin.backToDashboard', 'Quay lại Bảng điều khiển HR')}</span>
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                {t('admin.title', 'Cấu hình AI & Engine Đối sánh')}
              </h1>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1.5 leading-relaxed">
              {t('admin.subtitle', 'Quản lý mô hình bóc tách tài liệu (LLM) và vector embedding (BGE-M3 1024d) cho quy trình tuyển dụng thông minh.')}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 shrink-0">
            ...
            <span>{t('admin.adminMode', 'Chế độ Quản trị & HR')}</span>
          </div>
        </div>
